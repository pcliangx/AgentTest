...
| Provider 品牌图标 #79 | ✅ 已合并（PR #85）：新增 `provider-icon.tsx` 共享组件——原创 SVG 几何标记（Claude Code 珊瑚橙星爆 / Codex 近黑六边形纽结 / Kimi Code 深蓝新月）+ 品牌色入 `@theme` 令牌；全量替换 5 处头像位（Agent Directory 31px / Panel 头卡 31px / Settings 列表+详情 27/38px / Dispatch Picker 24px / Attention 抽屉 28px）；未知 Provider 降级为现有 mono 首字母块；图标 aria-hidden 不随状态变化，StatusDot 纪律不变；无契约改动 |
